...
- A NATS CLI is handy but optional. If you don't have it locally, run it from the
  bundled image. The default stack is **authenticated**, so present the demo
  admin creds (run from this directory):
  ```sh
  nats() { docker run --rm --network quik-nats -v "$PWD/auth:/c:ro" \
    natsio/nats-box nats -s nats:4222 --creds /c/admin.creds "$@"; }
  ```
  (For the no-auth stack, drop the `-v` and `--creds`. With a local CLI:
  `nats -s localhost:4222 --creds auth/admin.creds …`, since NATS is published on
  127.0.0.1:4222. Or browse it in a GUI: `docker compose --profile ui up -d nui`,
  then http://localhost:31311.)
...
This default stack is **authenticated** - NATS runs decentralised JWT and each
credential is scoped to exactly what it may write (throwaway demo creds under
[`auth/`](auth/); see [`auth/README.md`](auth/README.md)). For the open minimal
demo, `docker compose -f docker-compose.no-auth.yml up -d --build` instead.
...
whose `[liveness]` gate registers the backend while it stays alive - config in
[`auth/quik-register.toml`](auth/quik-register.toml); the gate decides registry
membership, while quik's own active health decides routing). If
...
- **Immediately:** quik's active-health probe to `checkout-2.svc:8080` starts
  ...
  ```
- **Within one TTL (~30 s):** the registrar's heartbeat is liveness-gated, so
  once the service stops answering its liveness probe the registrar stops
  refreshing the key; the lease expires and the watcher removes the member
  entirely.
  ```sh
